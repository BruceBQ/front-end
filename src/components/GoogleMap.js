...
const apiIsLoaded = (map, maps, cameras) => {
  if(cameras.length > 0){
    ...
    if (bounds.getNorthEast().equals(bounds.getSouthWest())) {
      let extendPoint1 = new maps.LatLng(bounds.getNorthEast().lat() + 0.01, bounds.getNorthEast().lng() + 0.01);
      let extendPoint2 = new maps.LatLng(bounds.getNorthEast().lat() - 0.01, bounds.getNorthEast().lng() - 0.01);
    ...
    return(
      ...
      >
        <MapControl map={this.map || null}
          controlPosition={this.maps ? this.maps.ControlPosition.RIGHT_BOTTOM : null}
        >
