...
class LocationsForm extends Component{
    ...
    render(){
        ...
        const provinceOptions = provinces.map( province => {
            return {
                value: province.province_code, label: province.name
            }
        })
        const districtOptions = districts.map( district => {
            return {
                value: district.district_code, label: district.name
            }
        })
        
        const communeOptions = communes.map( commune => {
            return {
                value: commune.commune_code, label: commune.name
            }
        })
        const errors = {}
        return(
            <div className="camera-detail ml-4">
                ...
                <div className="item">
                    ...
                    <div className="fix-width-input">
                        <TextInput name="hamlet" placeholder="Thôn(Xóm)/Tổ dân phố" type="text" onChange={this.onChange}  value={this.state.hamlet}/>
                    </div>
                </div>
                <div className="item">
                    <span className="info-title">Đường:</span>
                    <div className="fix-width-input">
                        <TextInput name="street" placeholder="Đường" type="text" onChange={this.onChange}  value={this.state.street}/>
                    </div>
                </div>
                <div className="item">
                    <span className="info-title">Nhóm:</span>
                    <div className="fix-width-input">
                        <TextInput name="group" placeholder="Nhóm" type="text" onChange={this.onChange}  value={this.state.group}/>
                    </div>
                </div>
                <div className="item">
                    <span className="info-title">Vị trí lắp đặt:</span>
                    <div className="fix-width-input">
                        <TextInput name="position" placeholder="Vị trí lắp đặt" type="text" onChange={this.onChange}  value={this.state.position}/>
                    </div>
                </div>
